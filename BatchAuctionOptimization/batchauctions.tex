...
Pairwise exchange rates between two tokens $ \tau_j $ and $ \tau_k $ will be denoted by $ p_{j|k} $,
meaning the price of one unit of $ \tau_j $ measured in units of $ \tau_k $.
...
\begin{enumerate}
  ...
  Hence, the trader expresses a willingness to a buy a certain amount of $\tau_j$ independent
  of the price per unit, but limits the total amount of $\tau_k$ that needs to be paid.
\end{enumerate}

\figref{fig:buy_sell_demand} illustrates the characteristic buy- and sell-demand profiles of the
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{
    Buy- and sell-demand profiles for limit orders (types 1/2/3).
    The graphs show the maximum amounts of tokens that can be bought and sold with respect to the
    exchange rate~$p_{j|k}$.
...
\begin{itemize}
  ...
  \begin{align*}
    w &= ( x - y / \pi ) \> p_j% \\
    %  &= x \cdot p_j - y \cdot \pi \cdot p_j \\
    %  &= v - y \cdot \pi \cdot p_k
    %\label{eq:trading_surplus}\notag
  \end{align*}

  \item \todo[inline]{Definition of welfare for all order types.}
\end{itemize}
